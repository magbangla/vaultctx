...
func Test_vaultctxFilePath(t *testing.T) {
	origHome := os.Getenv("HOME")
	os.Setenv("HOME", filepath.FromSlash("/foo/bar"))
	defer os.Setenv("HOME", origHome)

	expected := filepath.Join(filepath.FromSlash("/foo/bar"), ".vault", "vaultctx")
	v, err := vaultctxPrevCtxFile()
	if err != nil {
		t.Fatal(err)
	}
	if v != expected {
...
func Test_vaultctxFilePath_error(t *testing.T) {
	origHome := os.Getenv("HOME")
