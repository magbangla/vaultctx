...
func (k *vaultconfig) WithLoader(l Loader) *vaultconfig {
	k.loader = l
	return k
}

func (k *vaultconfig) Close() error {
	if k.f == nil {
		return nil
	}
	return k.f.Close()
...
func (k *vaultconfig) Parse() error {
	files, err := k.loader.Load()
	if err != nil {
		return errors.Wrap(err, "failed to load")
	}

	// TODO since we don't support multiple vaultconfig files at the moment, there's just 1 file
	f := files[0]
	...
	if k.rootNode.Kind != yaml.MappingNode {
		return errors.New("vaultconfig file is not a map document")
	}
	return nil
}

func (k *vaultconfig) Bytes() ([]byte, error) {
	return yaml.Marshal(k.rootNode)
}

func (k *vaultconfig) Save() error {
